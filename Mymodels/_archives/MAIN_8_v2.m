% difference with MAIN_8 : we do not look for alpha before,
% instead we find directly the max of U by adding the constraint
% P_k(und,asympt)>= tolP
    ...
    for Id_ech=unique(paramsHIV.IDech_id)'
        ...
        for pHIV=unique(paramHIV_ech.p)'
            ...
            for numKit=1:length(list_kits_to_consider)
                ...
                [cnn,~,msgCnn] = find_Cnn_kit(paramTab,mu,b,paramRho,f,kit,aprioriBndsC,alphaMax,afficherOutput,log_path_id);
