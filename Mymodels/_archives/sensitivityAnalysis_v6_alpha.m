        ...
        for p=vecP'
            ...
            if eval(cond_part_A)
                ...
                for numKit=1:length(list_kits_to_consider)
                    ...
                    if ~doNotRunModel
                        [alpha,P,~,msgAlpha_mod,elim_i] = findAlpha_v4(paramTab,paramRho,mu,b,f,kit,k_mod,afficherOutput,...
                    ...
                    tabAlpha(numSim,[k_mod,'_elim'])= table({elim_i}, 'VariableNames',{[k_mod,'_elim']});
                    ...
                    if ~contains(k,'h')
                        ...
                        if ~infElim.h && ~isempty(lineAlpha0) %if HIV is not eliminated, the model is similar to the one with hiv but p=0 (i.e. sg_sg is similar to sg_hsg(p=0))
                            ...
                            %elimCostbyModel(numKit,:).alphaK    = elimCostbyModel(numKit,:).alphaK;
                            %elimCostbyModel(numKit,:).msgAlphaK = msgAlpha_mod;
