function [alpha,Palpha,ES,msg,elim_i] = findAlpha_v3(paramTab,paramRho,mu,b,f,kit,mod,verbose,log_path,paramSolver,ampl_models_dir)
...
disp([' Calcul de alpha_',k, ' avec le modele ',mod])
if ~isequal(paramSolver.method_alpha,'dicho')
    while relancerCalculAlpha && nbRelances<nbRelancesTot
        ...
        knitro_options = '';
        knitro_options = [knitro_options, 'outmode=2 ms_enable=1 ms_maxsolves=6 feastol_abs=1e-6 maxtime_real=',num2str(timeLimit),' ms_maxtime_real=60 ms_outsub=1 ms_numthreads=4 ncvx_qcqp_init=0 ms_seed=',num2str(nbRelances)];
        ...
        [Palpha,ES,status] = P_mod_v7(paramTab,paramRho_temp,b,mu,f,'hscg',verbose,paramSolver,log_path,ampl_models_dir);
        disp([Palpha])
        vecP=[];
        for inf=k
    ...
    tabRecap_sim2 = array2table(tabRecap_sim,'VariableNames',["alpha",convertCharsToStrings(['P_',k]),"Ph","Ps","Pc","Pg","status"]);
    tabRecap_sim3 = sortrows(tabRecap_sim2,1);
else
    tabRecap_sim3 = table('Size',[0,7],...
        'VariableNames',["alpha",convertCharsToStrings(['P_',k]),"Ph","Ps","Pc","Pg","status"],...
        'VariableTypes',{'double','double','double','double','double','double','double'});
    
end

% On cherche alpha par dichotomie
if nbRelances>=nbRelancesTot || isequal(paramSolver.method_alpha,'dicho')
    ...
    while affinerAlpha && i<nbIterMax
        ...
        [Palpha,ES,status] = P_mod_v7(paramTab,paramRho_temp,b,mu,f,'hscg',verbose,paramSolver,log_path,ampl_models_dir);
        disp(['   test with alpha=',num2str(alpha_apriori)])
        %disp(Palpha)
        tabRecap_sim3(nbRelances+i,:) = array2table(([alpha_apriori,0,table2array(struct2table(Palpha)),status]),...
        ...
        if ~status
           ...
           alpha_inf = tabRecap_sim4(tabRecap_sim4.alpha < alpha_apriori,1);
           if isempty(alpha_inf)
               alpha_inf = table(0,'VariableNames',{'alpha'});
           end
           alpha_sup = tabRecap_sim4(tabRecap_sim4.alpha > alpha_apriori,1);
           new_alpha = (alpha_sup.alpha(1)-alpha_inf.alpha(end))/unifrnd(1,10) + alpha_inf.alpha(end);
           i=i+1;
        end
    end
    tabRecap_sim4 = sortrows(tabRecap_sim3,1);
    ...
    alpha_before_inf_stat = tabRecap_sim5(tabRecap_sim5.alpha<alpha,"status");
    alpha_before_sup_stat = tabRecap_sim5(tabRecap_sim5.alpha>alpha,"status");
