...
clear all;close all;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
disp('attention : selectionner toutes les maladies et enlever plot')
Ct=true; Ng=true; HIV=true; Syph=true; plotFigure=false;
nbSim = 100; sauvegardetousles = 25;
...
%%
vecCnn=[];vecC0=[];
for biasFactor = repmat([1,3,10],1,20)
    ...
    for i=(lastSim+1):(nbSim+lastSim+1)
        ...
        %cond= 2*paramCt.alpha<paramHIV.alpha;
        %t=t+1
        % end
        %%
        computeC0Cnn = 1;
        ...
        %N = 4;
        [bestStrat{i-lastSim,1},worstStrat(i-lastSim),tabRecapStrat,cnn,c0] =...
            findBestStrategy(computeC0Cnn,optFindCnnAndC0,optFindRhohat,N,paramTab,mu,b,biasFactor,plotFigure);
