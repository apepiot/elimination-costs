function [tabRhohat,tabC0,tabCnn,tabTimes] = findRhohat_v4(N,paramTab,mu,b,vecC,f)
...
if N>=2 %if there are at least 2 diseases
    ...
    for nm = 1:N2
        ...
        for c=vecC
            %disp(c);
            i=i+1;
            [rhohat,rhohatnm] = findRhohat2d_v4(c,typen,typem,vecRhohat1d(n).rhohat(i),...
                vecRhohat1d(m).rhohat(i),paramTab{n},paramTab{m},mu,b,f);
    ...
    for nmk = 1:N3
        ...
        for c=vecC
            ...
            [rhohat,rhohatnmk] = findRhohat3d_v4(c,typen,typem,typek,...
                rhohat1d_reshape(i,:),rhohat2d_reshape(i,:),...
...
if N==4
    ...
    for c=vecC
        ...
        [rhohat,rhohatnmkl] = findRhohat4d_v4(c,...
            ...
            rhohat1d_reshape(i),rhohat2d_reshape(i),rhohat3d_reshape(i),...
            paramTab{1},paramTab{2},paramTab{3},paramTab{4},mu,b,f);
      
        vecRhohat4d.rhohat(i) = rhohat;
