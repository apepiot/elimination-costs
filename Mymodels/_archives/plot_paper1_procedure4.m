function [] = plot_paper1_procedure4(vecRhomax1, c1, c2, alpha, vecC, inf, xlabels, ylabels,letter)
%procedure to plot same graphs for the paper 1, difference with the
%plot_paper1_procedure3: we add some labels/numbers on the axes
%for paper 2
    set(groot,'defaultAxesTickLabelInterpreter','latex');  
    %close all;
    %figure()
    limit_y = 1.3*alpha;
    a=area([(c1-(c2-c1)/2) (c1-(c2-c1)/2) c1 c1],[0 limit_y limit_y 0],'LineStyle','none'); a(1).FaceColor = [132/255, 151/255, 176/255];a.FaceAlpha = 0.5;
    hold on
    ...
    vecRhomax12 = vecRhomax1(vecC>=c1); vecC2 = vecC(vecC>=c1);
    ...
    text((c2+(c2-c1)/2)+ex*0.7,0-ey,'$c$','Interpreter','latex','FontSize',22,'FontWeight','bold','HorizontalAlignment', 'center')
    ...
    text(vecC(1)-2*ex,limit_y+2*ey,letter,'FontSize',20,'FontWeight','bold','HorizontalAlignment', 'center')
