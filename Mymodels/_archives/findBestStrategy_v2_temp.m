function [bestStrategy,worstStrategy,costStratTable,cleft,cright,costStratTable_sorted,tab_refined,tab,costElimTable] = findBestStrategy_v2_temp(cleft,cright,N,paramTab,mu,b,biasFactor,optFindRhohat)
...
if N==4
    ...
    decreaseC = abs(min(min(table2array(costElimTable(:,2:5))))-vecC(1))<precision;
    while(increaseC && t<1000)
        disp('increase boundary on the right')
        newC = vecC(end)+(vecC(end)-vecC(end-1));
        [newTab,~,~,~] = findRhohat_v5(N,paramTab,mu,b,newC,biasFactor);
        tab_refined = concatenateTabRhohat(tab_refined,newTab);
        [costElimTable] = createCostElimTable(tab_refined,tabcn,vecAlphas);
        vecC = tab_refined.c;
    ...
    t=0;
    while(decreaseC && t<1000)
        disp('decrease boundary on the left')
        newC = vecC(1)-(vecC(2)-vecC(1));
        [newTab,~,~,~] = findRhohat_v5(N,paramTab,mu,b,newC,biasFactor);
        tab_refined = concatenateTabRhohat(tab_refined,newTab);
        [costElimTable] = createCostElimTable(tab_refined,tabcn,vecAlphas);
        vecC = tab_refined.c;
    ...
    while (bestStrategyHasNotBeenFound)
        ...
        % 3. hasard
        k=0; err = optFindRhohat.Tol+1; iterMax=20; HIVEqu=0;
        while (costStratTable_sorted{1,'HIV'}==costStratTable_sorted{2,'HIV'} && k<iterMax && err>optFindRhohat.Tol)
            HIVelimRange  = find(elimOrder==3); %elimination range of HIV
            disElimBefore = elimDis(find(elimOrder==3)-1); %disease that is eliminated before
            listModsEqu = costStratTable_sorted{costStratTable_sorted{1,'HIV'}==costStratTable_sorted{:,'HIV'},1}; %models that have the same cost of elimination for HIV
            costElimHIV_1d = costElimTable{costElimTable{:,'model'}=="3",'HIV'};
            
            if 0
                %the model is a submodel of the other
                err=0;% on ne pourra pas faire mieux car les modeles sont egaux    
            else
                disp('affinage HIV')
