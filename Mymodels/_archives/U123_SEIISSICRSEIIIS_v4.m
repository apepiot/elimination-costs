function [U123,dU,P] = U123_SEIISSICRSEIIIS_v4(param1,param2,param3,...
    ...
    if(isequal(solveMethod,'ode45'))
        ...
        [~,ES_t] = ode45(@(t,Y)  ODE_SEIISSICRSEIIIS_v4(t,Y,beta1,gamma1,nu1,sigma1,eps1,...
    ...
    if(isequal(solveMethod,'fsolve'))
        ...
        [ES] = fsolve(@(Y)  ODE_SEIISSICRSEIIIS_v4(1,Y,beta1,gamma1,nu1,sigma1,eps1,...
                                         betaI2,betaC2,gamma2,sigma2,theta2,...
                                         beta3,sigma3,tau3,nu3,gamma13,theta3,gamma33,mu,b,rho),...
                                      Y0,options);    
    end
    ...
    P123 = min(f*PHIV + sum(ES([5:4:57,8:4:60]))/(b/mu),1); %v3.2%v4
    
    %below : exposed of STI and syphilis are not included in the prev
    %PHIV = sum(ES([2:4:78,3:4:79]))/(b/mu);
    %P123 = f*PHIV + sum(ES([9:4:17,29:4:77, 12:4:20,32:4:80]))/(b/mu);
