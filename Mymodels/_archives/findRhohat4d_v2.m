function [rhohat,rhohat4d] = findRhohat4d_v2(c,type1,type2,type3,type4,...
                                                rhohat1d,rhohat2d,rhohat3d,param1,param2,param3,param4,mu,b,...
                                                alpha1,alpha2,alpha3,alpha4,f)

    %This function finds the argmax of U in a 3-diseases model. 
    options = optimset('Display','off','TolFun',1e-2); %options for minsearch  
    if strcmp(type1,"SEIIS") && strcmp(type2,"SEIIS") && strcmp(type3,"SICR") && strcmp(type4,"SEIIIS")
        beta1=param1.beta;gamma1=param1.gamma;nu1=param1.nu;sigma1=param1.sigma;eps1=param1.eps;
        beta2=param2.beta;gamma2=param2.gamma;nu2=param2.nu;sigma2=param2.sigma;eps2=param2.eps;
        betaI3=param3.betaI;betaC3=param3.betaC;gamma3=param3.gamma;sigma3=param3.sigma;theta3=param3.theta;
        beta4=param4.beta;sigma4=param4.sigma;tau4=param4.tau;gamma14=param4.gamma1;...
            theta4=param4.theta;gamma34=param4.gamma3;nu4=param4.nu;

        %disp(['avant ode system']); toc
        opt.TolFun=1e-5;
        fun1234 = @(rho) -U1234_SEIIS2SICRSEIIIS(beta1,gamma1,nu1,sigma1,eps1,beta2,gamma2,nu2,sigma2,eps2,...
            betaI3,betaC3,gamma3,sigma3,theta3,beta4,sigma4,tau4,gamma14,theta4,gamma34,nu4,mu,b,rho,c,alpha1,alpha2,alpha3,alpha4,f,'fsolve',opt);
        
        rhohat1234 = min(max(fminsearch(fun1234,0,options),0),max([alpha1,alpha2,alpha3,alpha4]));
        %disp(['apres ode system']); toc
        list_rhohat1234 = max([rhohat1d,rhohat2d,rhohat3d,rhohat1234],0);
        U_list = U_SEIIS2SICRSEIIIS(beta1,gamma1,nu1,sigma1,eps1,beta2,gamma2,nu2,sigma2,eps2,...
            betaI3,betaC3,gamma3,sigma3,theta3,beta4,sigma4,tau4,gamma14,theta4,gamma34,nu4,mu,b,list_rhohat1234,c,...
            alpha1,alpha2,alpha3,alpha4,f);

        [~,imax] = max(U_list);
        rhohat = list_rhohat1234(imax);
        rhohat4d = rhohat1234;            
    else 
        error('Error. Models sorting. Cannot find any combination of models')
    end
end

