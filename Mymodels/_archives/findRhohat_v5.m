function [tabRhohat,tabC0,tabCnn,tabTimes,vecC] = findRhohat_v5(N,paramTab,mu,b,vecC,f)
...
for n=1:N
    type = paramTab{n}.modelType;
    [vecRhohat1d_n,c0n,cnn] = findRhohat1d_v4(type,paramTab{n},mu,b,vecC,f);
...
if N>=2 %if there are at least 2 diseases
    ...
    for nm = 1:N2
        ...
        while (rhohat<alphaMin & i<lC) %on commence par les plus grandes valeurs de c
            %disp(c); 
            c = vecC(lC-i);
            [rhohat,rhohatnm] = findRhohat2d_v7(c,typen,typem,vecRhohat1d(n).rhohat(lC-i),...
