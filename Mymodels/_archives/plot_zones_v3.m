...
infectionsMini = repelem({'STI', 'HIV', 'syph.'}, [nSEIIS nSICR nSEIIIS]);
if nSEIIS==2
    ...
    infectionsMini{2} = 'Ng';
elseif nSEIIS==1
    infections{1} = STIChoice;
    infectionsMini{1} = STIChoiceMini;
end
...
if N==1
    plot_paper1_procedure2(tab.rhohat, cs.cs1dis, cs.c0, vecAlpha, vecC, infectionsMini{1})
    yticks([ 0 vecAlpha(1)])
    yticklabels({'0',['$\rho_{',infectionsMini{1},'}^\prime$']})
elseif N>=2
   hold on
    p_rho1  = plot(vecC,tab.one(:,1),':','Linewidth',2,'Color','k','DisplayName',['$\hat\rho_{',infectionsMini{1},'}$']); %bleu/SEIIS 1
    p_rho2  = plot(vecC,tab.one(:,2),':','Linewidth',2,'Color',rouge,'DisplayName',['$\hat\rho_{',infectionsMini{2},'}$']); %jaune/SEIIS 1
    if N==2
        p_rhohat = plot(vecC(vecC<=cs.cs1dis),tab.rhohat(vecC<=cs.cs1dis),'-','Linewidth',3,'Color','k','DisplayName','$\hat\rho$'); %bleu/SEIIS 1
        hold on
        plot(vecC(vecC>cs.cs1dis),tab.rhohat(vecC>cs.cs1dis),'-','Linewidth',3,'Color','k','DisplayName','$\hat\rho$'); %bleu/SEIIS 1
        ...
        yticks([ 0  0.5 vecAlpha(cs.order(1)) vecAlpha(cs.order(2)) ])
        yticklabels({'0','0.5',['$\rho_{',infectionsMini{cs.order(1)},'}^\prime$'],...
            ['$\rho_{',infectionsMini{cs.order(2)},'}^\prime$']})   
        ylim([0,limy])
        if tabcn.one(1)<tabcn.one(2)
    ...
    if (N>=3)
        p_rho3   = plot(vecC,tab.one(:,3),':','Linewidth',2,'Color',bleu,'DisplayName',['$\hat\rho_{',infectionsMini{3},'}$']);
        if N==3
            %p_rhohat = plot(vecC,tab.rhohat,'-','Linewidth',3,'Color','k','DisplayName','$\hat\rho$'); 
            p_rhohat = plot(vecC(vecC<=cs.cs2dis),tab.rhohat(vecC<=cs.cs2dis),'-','Linewidth',3,'Color','k','DisplayName','$\hat\rho$'); %bleu/SEIIS 1
            plot(vecC(vecC<=cs.cs1dis & vecC>cs.cs2dis),tab.rhohat(vecC<=cs.cs1dis & vecC>cs.cs2dis),'-','Linewidth',3,'Color','k','DisplayName','$\hat\rho$'); %bleu/SEIIS 1
            hold on
            plot(vecC(vecC>cs.cs1dis),tab.rhohat(vecC>cs.cs1dis),'-','Linewidth',3,'Color','k','DisplayName','$\hat\rho$'); %bleu/SEIIS 1
            
            legend([p_rhohat,p_rho1,p_rho2,p_rho3],'Interpreter','latex')
            yticks([ 0 vecAlpha(cs.order(1)) vecAlpha(cs.order(2)) 0.5 vecAlpha(cs.order(3))])
            yticklabels({' ',['$\rho_{',infections{cs.order(1)},'}^\prime$'],...
                             ['$\rho_{',infectionsMini{cs.order(2)},'}^\prime$'],'0.5',...
                             ['$\rho_{',infections{cs.order(3)},'}^\prime$']})
        end
        if (N>=4)
            p_rho4   = plot(vecC,tab.one(:,4),':','Linewidth',2,'Color',vert,'DisplayName',['$\hat\rho_{',infectionsMini{4},'}$']); %vert/ SEIIIS
            
            if N==4
                p_rhohat = plot(vecC(vecC<=cs.cs4dis),tab.rhohat(vecC<=cs.cs4dis),'-','Linewidth',3,'Color','k','DisplayName','$\hat\rho$'); %bleu/SEIIS 1
                plot(vecC(vecC<=cs.cs3dis & vecC>cs.cs4dis),tab.rhohat(vecC<=cs.cs3dis & vecC>cs.cs4dis),'-','Linewidth',3,'Color','k','DisplayName','$\hat\rho$'); %bleu/SEIIS 1
                plot(vecC(vecC<=cs.cs2dis & vecC>cs.cs3dis),tab.rhohat(vecC<=cs.cs2dis & vecC>cs.cs3dis),'-','Linewidth',3,'Color','k','DisplayName','$\hat\rho$'); %bleu/SEIIS 1
                plot(vecC(vecC<=cs.cs1dis & vecC>cs.cs2dis),tab.rhohat(vecC<=cs.cs1dis & vecC>cs.cs3dis),'-','Linewidth',3,'Color','k','DisplayName','$\hat\rho$'); %bleu/SEIIS 1
                plot(vecC(vecC>cs.cs1dis),tab.rhohat(vecC>cs.cs1dis),'-','Linewidth',3,'Color','k','DisplayName','$\hat\rho$'); %bleu/SEIIS 1

                %p_rhohat = plot(vecC,tab.rhohat,'-','Linewidth',3,'Color','k','DisplayName','$\hat\rho$'); %bleu/SEIIS 1
                legend([p_rhohat,p_rho1,p_rho2,p_rho3,p_rho4],'Interpreter','latex')
                yticks([ 0 vecAlpha(cs.order(1)) vecAlpha(cs.order(2)) vecAlpha(cs.order(3)) 0.5 vecAlpha(cs.order(4))])
...
if N==2
    ...
    lim_xinf=tabcn.one(1)-(cs.c0-tabcn.one(1))/2;
    ...
    a1=area([cs.cs2dis cs.cs2dis cs.cs1dis cs.cs1dis],[0,limy,limy,0 ],...
        'LineStyle','None','DisplayName',['only ', num2str(infections{cs.order(1)}),' eliminated']); a1(1).FaceColor = gris;a1.FaceAlpha = 0.1;  
    a0=area([cs.cs1dis cs.cs1dis lim_xsup lim_xsup],[0,limy,limy,0 ],...
        'LineStyle','-','DisplayName','no disease eliminated'); a0(1).FaceColor = gris;a0.FaceAlpha = 0.0; a0(1).EdgeColor = gris*3;
...
xlabel("voluntary testing cost $c$","fontweight","bold",'FontSize',12,'Interpreter','latex')
ylabel("voluntary testing rate $\hat\rho$","fontweight","bold",'FontSize',12,'Interpreter','latex')
