function [U12,dU,P,P_inf] = U1234_SICTPSEIIISSEIIS2_v2(param1,param2,param3,param4,mu,b,paramRho,c,f,solvingMethod)
...
%alpha1=param1.alpha; alpha2=param2.alpha; alpha3=param3.alpha; alpha4=param4.alpha;
%rho = min([rho,alpha1,alpha2,alpha3,alpha4]);
Y0 = ones(7*4*4*5+1,1)/560;

restart = true; %new operation for fsolve or continue with ode45 to find the ES
iterNo = 0;
%solvingMethod='fsolve'; 
tspan=[0,50];
while restart && iterNo<100
    ...
    if isequal(solvingMethod,'fsolve')
        ...
        [ES] = fsolve(@(Y) ODE_SICTPSEIIISSEIIS2_v2(0,Y,...
            betaI1,betaC1,sigma1,theta1,zeta1,eta1,p1,...
            ...
            Y0,options);
        Y0 = rand(561,1); %random for the next iteration
    elseif isequal(solvingMethod,'ode45')
        ...
        [res] = ode45(@(t,Y) ODE_SICTPSEIIISSEIIS2_v3(t,Y,...
            betaI1,betaC1,sigma1,theta1,zeta1,eta1,p1,...
            beta2,sigma2,gamma23,tau2,theta2,...
            beta3,gamma3,nu3,eps3,sigma3,...
            beta4,gamma4,nu4,eps4,sigma4,...
    ...
    P_inf = zeros(1,4);
    P_inf(1) = 1-(sum(ES(1:7:554))+sum(ES(4:7:557)))/(b/mu);
    P_inf(2) = 1-sum(ES(reshape(repmat((1:35:554),7,1)+[0:6]',[1,560/5])))/(b/mu);
    P_inf(3) = 1-sum(ES(reshape(repmat((1:140:421),35,1)+[0:34]',[1,560/4])))/(b/mu);
    P_inf(4) = 1-sum(ES(1:140))./(b/mu);
    ...
    if (0) %test
        ...
        %alors que les infections ne doivent pas disparaitre:
...
%table2array(tabComp((tabComp.HIV1=="I" | tabComp.HIV1=="Ip" | tabComp.HIV1=="C" | tabComp.HIV1=="Cp") & tabComp.STI1~="IS" & tabComp.STI2~="IS","no"))'
PHIV = sum(ES([2:7:100,142:7:244,282:7:380, ... %I
    3:7:101,143:7:245,283:7:381, ... %C
    5:7:103,145:7:247,285:7:383, ... %Ip
    6:7:104,146:7:248,286:7:384, ... %Cp
    ]))/(b/mu);
P12 = min(f*PHIV + sum(ES([8:7:99,11:7:102,14:7:105,...
    141:7:239,144:7:242,147:7:245,...
    281:7:379,284:7:382,287:7:385]))/(b/mu),1);
P = max(P12,0);
U12 = 0;%rho*(P-c);
dU  = NaN;

% %for asymptomatic and not infected HIV
%     vecP=[];
%     for i=1:size(tabComp,1)
%         %if sum(table2array(tabComp(i,1:4))==["S","S","S","S"])<4 &&
%         %sum(table2array(tabComp(i,1:4))==["P","S","S","S"])<4 && sum(table2array(tabComp(i,1:4))==["T","S","S","S"])<4
%             if (table2array(tabComp(i,"HIV1"))~="I" && table2array(tabComp(i,"HIV1"))~="C"  &&...
%                     table2array(tabComp(i,"HIV1"))~="Ip" && table2array(tabComp(i,"HIV1"))~="Cp")
%                 if table2array(tabComp(i,"STI1"))~="IS" && table2array(tabComp(i,"STI2"))~="IS"
%                     if table2array(tabComp(i,"syph1"))~="S"
%                         vecP = [vecP,tabComp(i,:).no];
%                     else
%                         if table2array(tabComp(i,"STI1"))~="S"
%                             vecP = [vecP,tabComp(i,:).no];
%                         else
%                             if table2array(tabComp(i,"STI2"))~="S"
%                                 vecP = [vecP,tabComp(i,:).no];
%                             end
%                         end
%                     end
