...
if isequal(kit,{'HIV'}) %| strcmp(kit,'h')
    outP = sum(ES(HIV.asymptomaticHIV))/N;
end
if isequal(kit,{'syphilis'})
    outP = sum(ES(syphilis.asymptomatic))/N;
end
if isequal(kit,{'Ct'})
    outP = sum(ES(Ct.asymptomatic))/N;
end
if isequal(kit,{'Ng'})
    outP = sum(ES(Ng.asymptomatic))/N;
end
if isequal(kit,{'HIV','syphilis'})
    asympt = unique([HIV.asymptomaticHIV,syphilis.asymptomatic]);
    outP = sum(ES(asympt))/N;
end
...
if isequal(kit,{'HIV','syphilis','Ct','Ng'}) %| strcmp(kit,'hscg')
