...
function [plt] = plot_paper1_R_procedure2(vecR, c1, c2, vecC,R0)
%procedure to plot same graphs for the paper 1
    %close all;
    %figure()
    %upbound = (c2+(c2-c1)/2);
    limit_y = 1.2*R0;
    a=area([(c1-(c2-c1)/2) (c1-(c2-c1)/2) c1 c1],[0 limit_y limit_y 0],'LineStyle','none'); a(1).FaceColor = [132/255, 151/255, 176/255];a.FaceAlpha = 1;
    hold on
    ...
    %plot hatrho
    vecC1 = (c1-(c2-c1)/2):0.01:c1;
    plot(vecC(vecC<c2),R0*ones(length(vecC(vecC<c2)),1),'--','Color',[0.6,0.6,0.6],'LineWidth',3)
    plot(vecC1,ones(length(vecC1),1),'--','Color',[0.6,0.6,0.6],'LineWidth',3)

    vecR11 = vecR(vecC>=c1); vecC2 = vecC(vecC>=c1);
    plot(vecC2,vecR11,'-','LineWidth',5,'Color',[44/255, 63/255, 81/255])
    ...
    %ZONES I II III
    text((c1+vecC1(1))/2,0.9*limit_y,'III','Interpreter','latex','FontSize',20, 'FontWeight','bold','HorizontalAlignment', 'center')
    text((c1+c2)/2,0.9*limit_y,'II','Interpreter','latex','FontSize',20, 'FontWeight','bold','HorizontalAlignment', 'center')
    text((c2+(c2+(c2-c1)/2))/2,0.9*limit_y,'I','Interpreter','latex','FontSize',20, 'FontWeight','bold','HorizontalAlignment', 'center')
