...
#set infected_h_asympt_c := H_infectious diff C_IS;
#set infected_c_asympt_c := C_infectious diff C_IS;
var Prevalence_HC    = sum{k in infected_asympt_hc} (Y[k])/N_equ;
var Prevalence_H_HC  = sum{k in (infected_asympt_hc diff C_infectious)} (Y[k])/N_equ;
var Prevalence_HC_b  = f*Prevalence_H_HC + (Prevalence_HC-Prevalence_H_HC);#max(min(f*Prevalence_H_HC + (Prevalence_HC-Prevalence_H_HC),1),0);
...
minimize Cost: rho_hc*(c - Prevalence_HC_b);
