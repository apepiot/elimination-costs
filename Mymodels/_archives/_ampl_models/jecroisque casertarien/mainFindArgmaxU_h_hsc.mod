...
var rho_hsc>=inf_bnd_alpha, <= up_bnd_alpha;
...
set infected_hsc := H_infectious union S_infected union C_infected; 
set infected_asympt_hsc := infected_hsc diff C_IS;
var Prevalence_HSC = sum{k in infected_asympt_hsc} (Y[k])/N_equ;

#Prevalence of asymptomatic for HIV in the kit
set asympt_h := H_infectious diff C_IS;
var Prevalence_H_HSC = sum{k in asympt_h}(Y[k])/N_equ;

# Objective function to be minimized.
minimize Cost: rho_hsc*(c - Prevalence_H_HSC);
