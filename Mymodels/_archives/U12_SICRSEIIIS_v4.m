function [U12,dU,P] = U12_SICRSEIIIS_v4(param1,param2,mu,b,rho,c,f,solveMethod,optSolver)
    ...
    if(isequal(solveMethod,'ode45'))
        ...
        [~,ES_t] = ode45(@(t,Y) ODE_SICRSEIIIS_v4(t,Y,betaI1,betaC1,gamma1,sigma1,theta1,...
    ...
    if(isequal(solveMethod,'fsolve'))
        ...
        [ES] = fsolve(@(Y)  ODE_SICRSEIIIS_v4(1,Y,betaI1,betaC1,gamma1,sigma1,theta1,...
                                     beta2,sigma2,tau2,nu2,gamma12,theta2,gamma32,mu,b,rho),...
                                     Y0,options);
    end
    
    %P12 = max(sum(ES(end,[2,3,5:20]))/(b/mu),0);
    PHIV = sum(ES([2:4:18,3:4:19]))/(b/mu);             
    P12 = min(f*PHIV + sum(ES([5:4:17,8:4:20]))/(b/mu),1);
