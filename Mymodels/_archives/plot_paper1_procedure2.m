...
function [] = plot_paper1_procedure2(vecRhomax1, c1, c2, alpha, vecC, inf)
%procedure to plot same graphs for the paper 1
    ...
    vecRhomax12 = vecRhomax1.rhohat(vecC>=c1); vecC2 = vecC(vecC>=c1);
    ...
    ylim([min(vecRhomax1.rhohat),limit_y])
    %ylim([0,limit_y])
    ey = 0.1*alpha;
    ...
    %y-axis
    text(vecC(1)- ex,alpha,'$\rho\prime$','Interpreter','latex','FontSize',22, 'FontWeight','bold','HorizontalAlignment', 'center')
    text(vecC(1)- ex,0,'$0$','Interpreter','latex','FontSize',20, 'FontWeight','bold','HorizontalAlignment', 'center')
