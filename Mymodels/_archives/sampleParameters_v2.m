function [paramTab,mu,vecAlphas] = sampleParameters_v2(Ct,Ng,HIV,Syph,b)
%difference with sampleParameters: parameters (in particular beta) are
%calculated at the baseline voluntary testing rate
%create a table of parameters with a class structure
%beta_hiv calculated with the prevalence
...
rhob1  = 1/randPERT(6.77,8.14,9.51,1); %baseline voluntary testing rate
beta1  = R1/((sigma1*(gamma1*(1-eps1) + mu + nu1 + eps1*rhob1))./((mu + sigma1 + rhob1).*(gamma1 + mu + nu1).*(mu + nu1 + rhob1)));
[~,~,alpha1] = Rp_SEIIS_v4(beta1,nu1,eps1,sigma1,gamma1,mu,b,0); %compute rho'
...
rhob2  = 1/randPERT(6.77,8.14,9.51,1); %baseline voluntary testing rate
...
ratioBeta = randPERT(8.4,9.1,9.6,1); %betaI/betaC
rhobh     = 1/1/randPERT(1.6,1.9,2.2,1); %baseline voluntary testing rate
betaCSICR = RSICR*(sigmaSICR+gammaSICR+rhobh+mu)*(thetaSICR+rhobh+mu)/(ratioBeta*(thetaSICR+rhobh+mu)+sigmaSICR);
...
nuS     = 0;
rhobs   = 1/randPERT(2.06,2.48,2.89,1); %baseline voluntary testing rate
