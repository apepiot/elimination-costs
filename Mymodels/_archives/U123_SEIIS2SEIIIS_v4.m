function [U123,dU,P] = U123_SEIIS2SEIIIS_v4(param1,param2,param3,...
                                     mu,b,rho,c,f,solveMethod,optSolver)                                
    
    %syph (1:S,2:E,3:I1,4:I2,5:I3), STI1 (1:S,2:E,3:IA,4:IS), STI2                            
    % [ X1, 1, 1, 1]
    % [ X2, 2, 1, 1]
    % [ X3, 3, 1, 1]
    % [ X4, 4, 1, 1]
    ...
    if(isequal(solveMethod,'ode45'))
        ...
        [~,ES_t] = ode45(@(t,Y)  ODE_SEIIS2SEIIIS_v4(t,Y,beta1,nu1,eps1,sigma1,gamma1,...
    ...
    if(isequal(solveMethod,'fsolve'))
        ...
        [ES] = fsolve(@(Y)  ODE_SEIIS2SEIIIS_v4(1,Y,beta1,nu1,eps1,sigma1,gamma1,...
                                 beta2,nu2,eps2,sigma2,gamma2,...
                                 beta3,sigma3,tau3,nu3,gamma13,theta3,gamma33,mu,b,rho),...
                                 Y0,options);    
    end
    ...
    P = max(sum(ES([2:15,21:35,41:55]))/(b/mu),0); %prevalence do not include IS
    %P = max(sum(ES([3:5,8:20,23:25,28:80]))/(b/mu),0); %prevalence do not include E (for STI1,STI2 and/or syph)
