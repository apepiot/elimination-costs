function [tabRhohat,tabC0,tabCnn,tabTimes,vecC] = findRhohat_v6(N,paramTab,mu,b,vecC,f)
%for any combination of SICTP^n x SEIIIS^p x SEIIS^m, this
%function finds the argmax of single diseases models, two-disease
%models,...,N diseases models
%difference with findRhohat_v5:  findRhohatid_v4<-findRhohatid_v5 and
%SICR<-SICTP
tic; 
...
%     %disp(type);
%     [~,c0n,cnn] = findRhohat1d_v4(type,paramTab{n},mu,b,[],f); 
%     %Integrons les cnn au vecteur de c (si ils sont compris entre le min de
%     %vecC et le max de vecC)
%     if (cnn>min(vecC) && cnn<max(vecC))
%         vecC = sort([vecC,cnn]);
%     else %par defaut on met les bornes
%         
%     end
%     vecC0n = [vecC0n,c0n];
%     vecCnn = [vecCnn,cnn];
...
for n=1:N
    type = paramTab{n}.modelType;
    [vecRhohat1d_n,c0n,cnn] = findRhohat1d_v5(type,paramTab{n},mu,b,vecC,f);
...
if N>=2 %if there are at least 2 diseases
    ...
    for nm = 1:N2
        ...
        rhohat = 0; i=0;
        %for c=vecC
        while (rhohat<alphaMin & i<lC) %on commence par les plus grandes valeurs de c
            %disp(c); 
            c = vecC(lC-i);
            [rhohat,rhohatnm] = findRhohat2d_v5(c,typen,typem,vecRhohat1d(n).rhohat(lC-i),...
...
if N>=3
    ...
    for nmk = 1:N3
        ...
        while (rhohat<alphaMin & i<lC)
            c = vecC(lC-i);
            [rhohat,rhohatnmk] = findRhohat3d_v5(c,typen,typem,typek,...
...
if N==4
    ...
    %for c=vecC      
        ...
        [rhohat,rhohatnmkl] = findRhohat4d_v5(c,...
