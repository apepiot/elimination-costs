function [U,dU] = U_SICRSEIIIS(betaI1,betaC1,gamma1,sigma1,theta1,...
                               beta2,sigma2,tau2,nu2,gamma12,theta2,gamma32,...
                               mu,b,vecRho,c,alpha1,alpha2,f)
    ...
    for rho=vecRho       
        if rho<alpha1 && rho<alpha2 %R1(i)>1 && R2(i)>1
            U(i) = U12_SICRSEIIIS(betaI1,betaC1,gamma1,sigma1,theta1,beta2,sigma2,tau2,nu2,gamma12,theta2,gamma32,mu,b,rho,c,alpha1,alpha2,f,'fsolve',opt);
        elseif rho>=alpha1 && rho<alpha2 %R1(i)<=1 && R2(i)>1
            U(i) = U_SEIIIS(b, beta2, sigma2, gamma12, gamma32, tau2, theta2, nu2, mu, rho, c, alpha2);
        elseif rho>=alpha2  && rho<alpha1 %R1(i)>1 && R2(i)<=1
            U(i) = U_SICR(betaI1, betaC1, sigma1, gamma1, theta1, mu, rho, c, alpha1,f);
        else
            U(i) = -c*rho;
        end
        i=i+1;
