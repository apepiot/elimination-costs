...
if newSet 
    ...
    paramTab{3}.eta = 4; paramTab{3}.zeta=randPERT(46,60,71,1)/100;
...
% Other parameters relative to HIV
...
allParameters4d = {paramTab{3}.betaI,paramTab{3}.betaC,paramTab{3}.sigma,paramTab{3}.theta,paramTab{3}.zeta,paramTab{3}.eta,paramTab{3}.p,...
...
[res4dis] = ode45( @(t,Y) ODE_SICTPSEIIISSEIIS2_v5(t,Y,allParameters4d{:}),...
    tspan, Y0);
...
% Initial condition 
Y0 = ones(560,1)/560;
solveur = "fsolve"; %fsolve, ode45 knitro, knitro_ampl
tspan=[0,50];

N_th=b/mu;
restart=true; iterNo=0; 
while restart && iterNo<10
    tic;
    iterNo=iterNo+1
%     if iterNo>10
%         solveur="ode45";
%     end
    if solveur=="fsolve"
        ...
        [ES,fval,exitflag,output] = fsolve(@(Y) ODE_SICTPSEIIISSEIIS2_v5(0,Y,allParameters4d{:}),Y0,options);  
        Y0 = rand(560,1)*b/mu; %pour le coup d'après
        
    elseif solveur=="knitro"
        my_f = @(Y) ODE_SICTPSEIIISSEIIS2_v5(0,Y,allParameters4d{:});
    ...
    elseif solveur=="ode45"       
        [res] = ode45(@(t,Y) ODE_SICTPSEIIISSEIIS2_v5(t,Y,allParameters4d{:}),tspan, Y0);
        ES = res.y(:,end);
...
for simu=1:nbSimu
    ...
    %paramTab{1}.beta=0.1;
    %paramTab{2}.beta=0.1;
    %paramTab{3}.betaI=0.1;
    %paramTab{3}.betaC=0.1;
    %paramTab{4}.beta=0.01;
...
%     paramTab{2}.rhob=1;
    ...
    tic;
    [ES_knitro,~,P_inf_knitro,lastIter,optimFound,changeSolver] = P1234_SICTPSEIIISSEIIS2_v5(paramTab{3},paramTab{4},paramTab{1},paramTab{2},mu,b,...
