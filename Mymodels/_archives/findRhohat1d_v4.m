function [rhohat,c0n,cnn] = findRhohat1d_v4(type,param,mu,b,vecC,f)
    ...
    elseif isequal(type,'SICR')
        [~,c0n] = U1_SICR_v4(param,mu,b,0,0,f); %c2 = c0
        [~,cnn] = U1_SICR_v4(param,mu,b,param.alpha,0,f); %c1=c1
        i=1;
        for c=vecC 
            fun = @(rho) -U1_SICR_v4(param,mu,b,rho,c,f);
            rhohat(i) = min(max(fminsearch(fun,0,options),0),param.alpha);
            i=i+1; %c
        end
