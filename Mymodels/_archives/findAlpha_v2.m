function [alpha,P,ES,msg] = findAlpha_v2(paramTab,paramRho,mu,b,f,kit,verbose,log_path,paramSolver,ampl_models_dir)
...
while relancerCalculAlpha && nbRelances<nbRelancesTot
    ...
    knitro_options = '';
    knitro_options = [knitro_options, 'outmode=2 ms_enable=1 ms_maxsolves=6 feastol=1e-8 maxtime_real=',num2str(timeLimit),' ms_maxtime_real=60 ms_outsub=1 ms_numthreads=4 ncvx_qcqp_init=0 ms_seed=',num2str(nbRelances)];
    ...
    rho = ampl.getVariable(['rho_',nameRho]);
    df = rho.getValues;
    a = df.val;
    alpha = a{:};
    ...
    Pk_ampl = ampl.getVariable(['Prevalence_',upper(nameRho)]);
