function [tabcs] = findThresholds_v2(nSEIIS,nSICR,nSEIIIS,tab,vecAlpha,vecC)
...
%             end
            ...
            tabcs.cs2dis = max(vecC(tab.rhohat>=alphai|abs(tab.rhohat-alphai)<100*eps));%+step/2; %when 2 diseases are eliminated
            tabcs.cs1dis = max(vecC(tab.rhohat>=alphaj|abs(tab.rhohat-alphaj)<100*eps));%+step/2; %when only one disease is eliminated
        ...
        elseif (N==3)
            ...
            tabcs.cs1dis = max(vecC(tab.rhohat>=alphak));
            tabcs.cs2dis = max(vecC(tab.rhohat>=alphaj));
            tabcs.cs3dis = max(vecC(tab.rhohat>=alphai));
    ...
    %         tabcs.cs1dis = max(vecC(vecRhohat>=minalpha))+pasC/2; %when only one disease is eliminated
        elseif (N==4)
            ...
            tabcs.cs1dis = max(vecC(tab.rhohat>=alphal));
            tabcs.cs2dis = max(vecC(tab.rhohat>=alphak));
            tabcs.cs3dis = max(vecC(tab.rhohat>=alphaj));
            tabcs.cs4dis = max(vecC(tab.rhohat>=alphai));
        else
            error('the case where N>4 has not been coded yet')
        end

        tabcs.c0     = min(vecC(tab.rhohat==0));
    end
end
