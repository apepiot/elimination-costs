...
var Prevalence_H_HG = sum{k in (infected_asympt_hg diff G_infectious)} (Y[k])/N_equ;
var Prevalence_HG_b  = f*Prevalence_H_HG + (Prevalence_HG-Prevalence_H_HG);#max(min(f*Prevalence_H_HG + (Prevalence_HG-Prevalence_H_HG),1),0);

# Objective function to be minimized.
minimize Cost: rho_hg*(c - Prevalence_HG_b);
