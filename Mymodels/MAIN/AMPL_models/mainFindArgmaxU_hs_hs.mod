...
var Prevalence_H_HS  = sum{k in (infected_asympt_hs diff S_infected)} (Y[k])/N_equ;
var Prevalence_HS_b  = f*Prevalence_H_HS + (Prevalence_HS-Prevalence_H_HS);
#var Prevalence_HS_b  = max(min(f*Prevalence_H_HS + (Prevalence_HS-Prevalence_H_HS),1),0);


# Objective function to be minimized.
minimize Cost: rho_hs*(c - Prevalence_HS_b);
