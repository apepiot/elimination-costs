%% plot
%change with plot_zones_v3bis: lines stops when the disease is eliminated 
% functions plot
close all
FontSizeGlobal = 14;
...
elseif nSEIIS==1
    ...
    if strcmp(paramTab{1}.disease,'Ct')
        infectionsMini{1} = 'c';
    else
        infectionsMini{1} = 'g';
    end
...
elseif N>=2
    ...
    %test
    p_rho1  = plot(vecC(tab.one(1).rhohat<vecAlphas(1)),tab.one(1).rhohat(tab.one(1).rhohat<vecAlphas(1)),':','Linewidth',2,'Color','k','DisplayName',['\fontsize{22}{0}\selectfont$\hat\rho$\fontsize{14}{0}\selectfont$_{',infectionsMini{1},'}$']); %bleu/SEIIS 1    
    %horizontal dotted line from minC to c of elim
    plot(vecC(tab.one(1).rhohat>=vecAlphas(1)),tab.one(1).rhohat(tab.one(1).rhohat>=vecAlphas(1)),':','Linewidth',2,'Color','k','HandleVisibility','off');

    %p_rho2  = plot(vecC,tab.one(2).rhohat,'--','Linewidth',2,'Color',rouge,'DisplayName',['$\hat\rho_{',infectionsMini{2},'}$']);
    p_rho2  = plot(vecC(tab.one(2).rhohat<vecAlphas(2)),tab.one(2).rhohat(tab.one(2).rhohat<vecAlphas(2)),'--','Linewidth',2,'Color',rouge,'DisplayName',['\fontsize{22}{0}\selectfont$\hat\rho$\fontsize{14}{0}\selectfont$_{',infectionsMini{2},'}$']); %jaune/SEIIS 1
    %horizontal dotted line from minC to c of elim
    plot(vecC(tab.one(2).rhohat>=vecAlphas(2)),tab.one(2).rhohat(tab.one(2).rhohat>=vecAlphas(2)),':','Linewidth',2,'Color',rouge,'HandleVisibility','off');

    if N==2
        %%
        %p_rhohat = plot(vecC(vecC<=cs.cs1dis),tab.rhohat.rhohat(vecC<=cs.cs1dis),'-','Linewidth',3,'Color','k','DisplayName','$\hat\rho$'); %bleu/SEIIS 1
        p_rhohat = plot(vecC(vecC<=cs.cs1dis & vecC>cs.cs2dis),tab.rhohat.rhohat(vecC<=cs.cs1dis & vecC>cs.cs2dis),'-','Linewidth',3,'Color','k',...
...
if N==2
  ...
  %%  
    ...
    text('position',[lim_xinf-ex paramTab{2}.alpha+ey 0],'interpreter','latex','string',['\fontsize{22}{0}\selectfont$\rho$\fontsize{14}{0}\selectfont$_{',infectionsMini{2},'}^{\prime}$'],'HorizontalAlignment', 'left');
    text('position',[lim_xinf-ex paramTab{1}.alpha+ey 0],'interpreter','latex','string',['\fontsize{22}{0}\selectfont$\rho$\fontsize{14}{0}\selectfont$_{',infectionsMini{1},'}^{\prime}$'],'HorizontalAlignment', 'left');
...
text('position',[lim_xinf-ex limy 0],'interpreter','latex','string','\fontsize{22}{0}\selectfont$\hat\rho$','HorizontalAlignment', 'center');
...
text('position',[lim_xsup+0.8*ex -ey 0],'interpreter','latex','string','\fontsize{22}{0}\selectfont$c$','HorizontalAlignment', 'center');
