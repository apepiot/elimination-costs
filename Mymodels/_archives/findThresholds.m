function [tabcs] = findThresholds(nSIS,nSIR,nSICAT, tab, vecAlpha, beta, gamma, s, b, mu,vecC)
    N = nSIS+nSIR+nSICAT;
    if(N==1)
        if(nSIS==1)
            tabcs.cs1dis = (gamma+mu)/beta-1;
            tabcs.c0     = -tabcs.cs1dis;
        end
        if(nSIR==1)
            tabcs.cs1dis = -mu./beta.*(1-(gamma+mu)/beta);
            tabcs.c0     =  mu./beta.*(beta/(gamma+mu)-1);
        end
    end
    if(N>1)
        if(N==2)
