...
VTunderART        = 1;
afficherOutput    = 0;
vecRounds_global  = 96; %5:8;%45+[11,12,13];
kk                = 15;          %premier kit à lancer sur les 1:15
nosKitsAlpha      = 15; %[11,12,13,15];  %[1,5,6,7,11,12,13,15] only HIV
nosKitsCnn        = nosKitsAlpha;
calculer_alpha    = 0; %if f>1 : alphas already computed
cond_part_A       = '1'; %'pHIV==0.1 && noEch==2'; %'1' means all.
calculer_costs    = 1;
cond_part_Cnn     = 'noEch>218';
ecraser_elimCosts = 0;
parametrizationNo = 4;
%--------------------------------------------------------

addpath 'C:/Program Files (x86)/Artelys/AMPL 13.1.20220703/amplapi/examples/matlab';
%addpath 'C:\Users\pepiot\Documents\Artelys\AMPL-13.1.20220703-Win-64\amplapi\examples\matlab';
...
log_path   = ['../ParameterAnalysis/Resultats_knitro/parametrization_',num2str(parametrizationNo),'/'];
...
pathKnitroLog = ['../ParameterAnalysis/Resultats_knitro/parametrization_',num2str(parametrizationNo),'/knitro_out_rounds/',backUpFolder,'/'];

%addpath './Initialisation';
%addpath([pwd,'./AMPL_models'])
setupOnce;
