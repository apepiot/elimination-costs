function [U12,dU,P] = U12_SEIISSEIIIS(beta1,gamma1,nu1,sigma1,eps1,...
                                     beta2,sigma2,tau2,theta2,gamma12,gamma32,nu2,...
    ...
    if(isequal(solveMethod,'ode45'))    
        ...
        [~,ES_t] = ode45(@(t,Y)  ODE_SEIISSEIIIS_3(t,Y,beta1,nu1,eps1,sigma1,gamma1,...
    ...
    if(isequal(solveMethod,'fsolve'))
        ...
        [ES] = fsolve(@(Y)  ODE_SEIISSEIIIS_3(1,Y,beta1,nu1,eps1,sigma1,gamma1,...
                                     beta2,sigma2,tau2,nu2,gamma12,theta2,gamma32,mu,b,rho),...
                                     Y0,options);
    end
    P = max(sum(ES(2:15))/(b/mu),0); %prevalence does not include symptomatics)
