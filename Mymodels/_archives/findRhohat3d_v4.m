function [rhohat, rhohat3d] = findRhohat3d_v4(c,type1,type2,type3,...
                                                rhohat1d,rhohat2d,...
    ...
    if strcmp(type1,"SEIIS") && strcmp(type2,"SEIIS") && strcmp(type3,"SICR")
        fun123 = @(rho) -U123_SEIIS2SICR_v4(param1,param2,param3,mu,b,rho,c,f,'fsolve',opt);
        rhohat123 = min(max(fminsearch(fun123,0,options),0),max([alpha1,alpha2,alpha3]));
        list_rhohat123 = max([rhohat1d,rhohat2d,rhohat123],0);
        U_list = U_SEIIS2SICR_v4(param1,param2,param3,mu,b,list_rhohat123,c,f);
        [~,imax] = max(U_list);
    ...
    elseif strcmp(type1,"SEIIS") && strcmp(type2,"SICR") && strcmp(type3,"SEIIIS")  
        fun = @(rho) -U123_SEIISSICRSEIIIS_v4(param1,param2,param3,...
                                     mu,b,rho,c,f,'fsolve',opt);
        rhohat123 = min(max(fminsearch(fun,0,options),0),max([alpha1,alpha2,alpha3])); %argmaxU12
        list_rhohat123 = max([rhohat1d,rhohat2d,rhohat123],0); %candidates for Umax
        U_list = U_SEIISSICRSEIIIS_v4(param1,param2,param3,mu,b,list_rhohat123,c,f);
        [~,imax] = max(U_list);
        rhohat   = list_rhohat123(imax);
        rhohat3d = rhohat123; 
    else 
