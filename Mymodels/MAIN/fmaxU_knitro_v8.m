function [rhohat,Cval,ES,P,msg,ampl] = fmaxU_knitro_v8(kit,subKit,mod,subMod,paramTab,paramRho,f,mu,b,c,optSolver,ampl,newmod,ampl_models_dir)
mySeed = optSolver.seed;
%addpath 'C:/Users/Moi/Documents/IPLESP/These/Codes/multi-voluntary-testing/Mymodels/AMPL_models/';
addpath(ampl_models_dir);
log_path = '../tests'; %à changer ?
...
if ampl==0 || newmod==1          %initializing
    ...
    ampl = assigningParametersToAMPL_v3(paramTab,paramRho,mu,b,ampl,indexKit(subKit));
    up_bnd_P0  = ampl.getParameter('bnd_sup_0');
    up_bnd_P0.setValues(optSolver.tolP0);
    
    if contains(k,'h')
        bias  = ampl.getParameter('f');
        bias.setValues(f);
    end
end
...
ps_0_ampl.setValues(int8(contains(subMod,'s')));
pc_0_ampl  = ampl.getParameter('p_c_0');
pc_0_ampl.setValues(int8(contains(subMod,'c')));
pg_0_ampl  = ampl.getParameter('p_g_0');
pg_0_ampl.setValues(int8(contains(subMod,'g')));
ph_0_ampl  = ampl.getParameter('p_h_0');
ph_0_ampl.setValues(int8(contains(subMod,'h')));

up_bnd_alpha = ampl.getParameter('up_bnd_alpha');
up_bnd_alpha.setValues(optSolver.up_bnd_alpha);
inf_bnd_alpha = ampl.getParameter('inf_bnd_alpha');
inf_bnd_alpha.setValues(optSolver.inf_bnd_alpha);
...
[~,~] = mkdir(log_path);
knitro_options = '';
knitro_options = [knitro_options, 'outmode=2 ms_enable=1 ms_maxsolves=4 feastol_abs=1e-6 maxtime_real=',num2str(optSolver.timeSolver),...
    ' ms_maxtime_real=',num2str(optSolver.timeSolver),' ms_outsub=1 ms_numthreads=4 ncvx_qcqp_init=0 ms_seed=',num2str(mySeed)];
...
if status ~= 0 
    disp(['    * Non-optimal status (',num2str(status),'), check multi-start procedure. *']);
    %ampl.close();
    %return;
    %break;
end
